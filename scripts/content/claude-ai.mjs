...
// Rather than hallucinating a number.`,
        ...
        quiz: [
          ...
          {
            question: 'What does the "HHH" framework stand for in Claude\'s training goals?',
            options: ['Helpful, Honest, Human', 'Helpful, Harmless, Honest', 'Human, Harmless, Honest', 'High-quality, Honest, Helpful'],
            correctIndex: 1,
          },
        ],
  ...
  messages: [
    {
      ...
      content: 'Add pagination to the /api/courses endpoint.'
        ...
        quiz: [
          ...
          {
            question: 'How can you replicate a Claude Project\'s persistent context when using the raw API?',
            options: ['You cannot, Projects are API-exclusive', 'Use the system parameter to inject the same instructions/context on every call', 'Projects work automatically with the API', 'Only via the claude.ai web interface'],
            correctIndex: 1,
          },
        ],
...
// — and it renders live in the Artifact panel`,
        ...
        quiz: [
          ...
          {
            question: 'Are Artifacts available through the raw Anthropic API?',
            options: ['Yes, identical to claude.ai', 'No, Artifacts are a claude.ai-only feature', 'Only for Enterprise API customers', 'Only for HTML, not React'],
            correctIndex: 1,
          },
        ],
  ...
  if (block.type === 'thinking') {
    ...
    console.log('Final answer:', block.text);
        ...
        quiz: [
          ...
          {
            question: 'What API parameter enables Extended Thinking and controls its reasoning budget?',
            options: ['reasoning: true', 'thinking: { type: "enabled", budget_tokens: N }', 'mode: "extended"', 'max_tokens: "unlimited"'],
            correctIndex: 1,
          },
        ],
      ...
      content: \`Analyse this document and summarise key findings:\n\n\${documentText}\`,
        ...
        quiz: [
          ...
          },
          {
            ...
            options: [
              'Claude crashes on long inputs',
              'Content in the middle of a very long context may receive less attention than content at the start/end',
              'Claude deletes the middle of long documents',
              'Tokens in the middle cost more',
            ],
            correctIndex: 1,
          },
        ],
...
console.log(followUp.content[0].text); // "Your name is Ravi."`,
        ...
        quiz: [
          ...
          {
            question: 'Why must the full conversation history be sent on every Anthropic API call for a multi-turn chat?',
            options: [
              'It is optional, the API remembers past calls',
              'The API is stateless — it has no memory of previous calls',
              'It reduces token cost',
              'It is only needed for Opus',
            ],
            correctIndex: 1,
          },
        ],
...
const issues = JSON.parse(response.content[0].text);`,
        ...
        quiz: [
          ...
          {
            question: 'Where should variable content like user input generally be placed in an XML-tagged prompt?',
            options: ['Always first', 'Last, after fixed instructions', 'It does not matter', 'Never included in the prompt'],
            correctIndex: 1,
          },
        ],
...
if (toolResponse.stop_reason === 'tool_use') {
  ...
  console.log(finalResponse.content[0].text);
        ...
        quiz: [
          ...
          {
            question: 'What method do you call on the Anthropic SDK client to receive tokens as they are generated?',
            options: ['client.messages.create()', 'client.messages.stream()', 'client.messages.fetch()', 'client.messages.live()'],
            correctIndex: 1,
          },
        ],
    ...
    concepts: [
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'What is a strong use case for asking Claude to adjust tone on a draft email?',
            options: [
              'Rewriting a frustrated email diplomatically while preserving the main point',
              'Deleting the email entirely',
              'Translating it into another language only',
              'Generating a completely unrelated email',
            ],
            correctIndex: 0,
          },
        ],
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'What is a good conversational strategy for learning a complex topic with Claude?',
            options: [
              'Ask one huge question covering everything at once',
              'Start broad, then drill down into specifics, then apply the concept',
              'Only ask yes/no questions',
              'Avoid follow-up questions entirely',
            ],
            correctIndex: 1,
          },
        ],
    ...
    concepts: [
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'Why is Claude Haiku recommended for scheduled report generation?',
            options: [
              'It is the only model with cron support',
              'It is fast, cheap, and reliable for turning raw data into narratives at recurring intervals',
              'It has the largest context window',
              'It cannot be scheduled at all',
            ],
            correctIndex: 1,
          },
        ],
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'How does a Claude code review bot deliver its feedback on a pull request?',
            options: [
              'It directly modifies the code without review',
              'It posts the review as a PR comment via the GitHub API (e.g. Octokit)',
              'It sends a private email to the repo owner only',
              'It cannot post feedback anywhere',
            ],
            correctIndex: 1,
          },
        ],
    ...
    concepts: [
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'Where should the <output_format> tag typically be placed in a Claude prompt for maximum effect?',
            options: ['At the very beginning', 'Last, after all other sections', 'In the middle', 'It should not be used at all'],
            correctIndex: 1,
          },
        ],
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'What is the purpose of a CAN/CANNOT capability declaration in a production system prompt?',
            options: [
              'It has no real effect on behaviour',
              'It explicitly bounds what the assistant should and should not do, preventing scope creep',
              'It only affects the pricing tier',
      ...
      },
      {
        ...
        quiz: [
          ...
          {
            question: 'What is "red-teaming" a system prompt in the meta-prompting context?',
            options: [
              'Deleting the system prompt entirely',
              'Asking Claude to generate adversarial inputs that would break the prompt, then fixing it against those cases',
              'Running the prompt on a different model',
