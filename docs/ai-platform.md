...
# Admin analytics and reports (phase 10)

Completes the admin surface. Prompt moderation (phase 5) and trend snapshots
(phase 6) already existed; this adds platform analytics and a reports queue, and
surfaces anything waiting on a human from the admin dashboard.

## Aggregate only

`/api/admin/analytics` counts usage; it never itemises it against a person.
Moderation legitimately needs to know who reported what — one account flagging
everything is exactly the pattern moderation exists to catch — so the reports
queue shows reporter names, and nothing else in the admin surface does. Nobody
gets a per-user readout of what people are asking AI about.

## What it reports

- **AI usage** — total and 30-day runs, a zero-filled daily chart, most-used
  actions with average duration, runs by provider, and connections per provider
  with how many have tested working.
- **Real vs demo** — the share of runs that returned sample data because no
  provider was connected. A high number is a product problem (the connection
  step is not obvious enough), so it is stated plainly rather than buried.
- **Library** — prompts by status, open reports, and the most-used published
  prompts with their ratings.
- **Engagement** — active users over 7 and 30 days (counted from recorded
  activity, not page loads), streak distribution, recorded learning hours, posts
  generated by platform, trend snapshots.

Built-in template ids resolve to their human titles; library runs, which are
stored as `library:<id>`, are grouped under "Library prompt" rather than shown as
raw ids.

## Reports queue

`/admin/reports` lists reports by state with the prompt, the reason, the
reporter's note and who sent it. Two closing actions:

- **Resolved** — the complaint was valid and has been acted on
- **Dismissed** — the complaint does not hold

Either decrements the prompt's `reportCount`, guarded so closing the same report
twice cannot push the counter negative. `reportCount` therefore always means
"open complaints", not "complaints ever received". A report whose prompt was
deleted is kept and shown as such, so the record survives the content.

## Routes added

| Route | What |
|---|---|
| `GET /api/admin/analytics` | the aggregate figures above |
| `GET /api/admin/reports` | reports by `?status=open\|resolved\|dismissed\|all` |
| `PATCH /api/admin/reports/[id]` | resolve or dismiss one |
| `/admin/analytics` | the dashboard |
| `/admin/reports` | the queue |

The admin dashboard now shows a banner when prompts are waiting for review or
reports are open, so work never sits unseen in a sub-page.
