...
const RATINGS = [
  { key: 'forgot', label: 'Forgot', color: 'bg-red-500 hover:bg-red-600' },
  { key: 'hard', label: 'Hard', color: 'bg-amber-500 hover:bg-amber-600' },
  { key: 'good', label: 'Good', color: 'bg-indigo-600 hover:bg-indigo-700' },
...
export default function RevisePage() {
  ...
  if (status !== 'authenticated') {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <p className="mt-2 text-slate-600">Login karke apne seekhe concepts spaced-repetition se revise karo.</p>
        <Link href="/login?callbackUrl=/revise" className="mt-6 inline-block rounded-lg bg-indigo-600 px-6 py-3 font-semibold text-white hover:bg-indigo-700">
          Login
        </Link>
      </div>
    );
  ...
  if (cards === null) return <p className="mx-auto max-w-2xl px-4 py-12 text-slate-400">Loading cards…</p>;

  if (cards.length === 0) {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        <p className="text-5xl">✅</p>
        <h1 className="mt-4 text-2xl font-bold">Sab revise ho gaya!</h1>
        <p className="mt-2 text-slate-600">
          {done > 0 ? `${done} cards reviewed. ` : ''}Abhi koi card due nahi. Naye concepts padho — wo automatically revision ke liye schedule honge.
        </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-500">Yaad hai? Mann mein recall karo, phir reveal karo.</p>
          ...
          <div className="mt-6 space-y-4">
            {card.keyPoints.length > 0 && (
              <div className="rounded-xl bg-slate-50 p-4">
                <p className="mb-2 text-sm font-semibold">Key points</p>
      ...
      {revealed && (
        <div className="mt-5">
          <p className="mb-2 text-center text-sm text-slate-500">Kitna yaad tha?</p>
