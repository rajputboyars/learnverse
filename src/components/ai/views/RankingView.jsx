...
export default function RankingView({ data }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="Ranking" icon="chart-line">
        <TableWrap>
      ...
      {!!data.decliningSkills?.length && (
        <Section title="Losing ground" icon="trend-down">
      ...
      {!!skills.length && (
        <Section title="Suggested learning order" icon="list-check">
          <BulletList
            items={[...skills]
              .sort((a, b) => (a.learnOrder ?? 99) - (b.learnOrder ?? 99))
              .map((s) => `${s.name} — ${s.difficulty || 'unrated'}${s.resources?.length ? ` (${s.resources.join(', ')})` : ''}`)}
          />
        </Section>
      )}
      ...
      {data.recommendation && (
        <Card className="border-indigo-200 bg-indigo-50">
          <p className="text-sm font-semibold text-indigo-900">What to do about it</p>
