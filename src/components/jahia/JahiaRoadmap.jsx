...
/**
 * JAHIA DEVELOPER ROADMAP — every module as a stage, grouped into the four
 * bands, each marked ✓ completed, ● current or ○ upcoming.
 *
 * @param stages [{ id, stage, title, level, estimatedMinutes, concepts, done, total }]
 */
export default function JahiaRoadmap({ stages, tracking }) {
  const { pick } = useLang();
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-baseline gap-3">
        <h2 className="text-xs font-extrabold uppercase tracking-[0.2em] text-slate-500">
          {pick('Jahia Developer Roadmap', 'Jahia Developer Roadmap')}
        </h2>
      ...
      <div className="mt-5 grid grid-cols-[minmax(0,1fr)] gap-6 md:grid-cols-2 xl:grid-cols-4">
        {BANDS.map((band) => {
          const list = stages.filter((s) => s.level === band.key);
                  ...
                  return (
                    <li key={s.id} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        ...
                        {i < list.length - 1 && <span className="w-px flex-1 bg-slate-200" />}
                      </div>
                      <div className="min-w-0 flex-1 pb-3">
                        ...
                        <span className="mt-0.5 flex flex-wrap gap-x-2 text-[11px] text-slate-400">
                          ...
                          <span>
                            {tracking ? `${s.done}/${s.total}` : s.total} {pick('lessons', 'lessons')}
                          </span>
